...
# model_checkpoint_path = "models\\32_my_checkpoint.pth.tar"    // working the best till now
model_checkpoint_path = "models\\2L_ALL_30_my_checkpoint.pth.tar" # best what we have seen trained on most of the data use     2 layered
# model_checkpoint_path = "D:\\Minor2\\Work\\AutoImageCaption\\New\\my_checkpoint.pth.tar"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model, dataset = load_model(model_checkpoint_path, device)
def generate_caption(image_path, model= model, dataset=dataset, device=device):
    """
